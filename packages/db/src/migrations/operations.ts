  ...
  },
  {
    ...
    sql: `
      ...
      CREATE TABLE pending_approvals (
        ...
        updated_at TEXT NOT NULL,
        kind TEXT NOT NULL DEFAULT 'file_operation'
          CHECK (kind IN ('file_operation', 'pi_tool_call', 'docker_operation', 'share_operation', 'storage_operation', 'vm_operation'))
      );
      ...
      CREATE TABLE IF NOT EXISTS vm_operations (
        id TEXT PRIMARY KEY,
        approval_id TEXT REFERENCES pending_approvals(id) ON DELETE SET NULL,
        action TEXT NOT NULL CHECK (action IN ('create', 'start', 'shutdown', 'stop', 'restart', 'pause', 'resume', 'reset', 'snapshot', 'delete', 'console')),
        target_id TEXT NOT NULL,
        status TEXT NOT NULL CHECK (status IN ('proposed', 'approved', 'applied', 'failed')),
        metadata_json TEXT NOT NULL DEFAULT '{}',
        created_at TEXT NOT NULL,
      ...
      CREATE INDEX IF NOT EXISTS idx_vm_operations_created_at ON vm_operations(created_at);

      CREATE TABLE IF NOT EXISTS vm_console_authorizations (
        id TEXT PRIMARY KEY,
      ...
      CREATE INDEX IF NOT EXISTS idx_vm_console_authorizations_status_expires_at
